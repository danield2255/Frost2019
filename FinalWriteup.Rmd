...
- id: Spotify API
  title: Spotify API Audio Analysis
  url: https://developer.spotify.com/
- id: Grammy
  title: Grammy Award Winners
  url: https://www.grammy.com/grammys/awards/61st-annual-grammy-awards
- id: RIAA
  title: RIAA Certifications
  url: https://www.riaa.com/gold-platinum/
- id: spotify 
  title: Spotify US Weekly Top Streams
  url: https://spotifycharts.com/regional/us/weekly
- id: ultimate guitar
  title: Ultimate Guitar Chord Data
...
```{r}
allArtistReg = read_csv("Data/allArtistReg.csv")
unstandardizedMusicDf = read_csv("Data/unstandardizedMusicDf.csv")
...
## Abstract
  Writing a song can be likened to simply making a series of choices, and just like any other decision-making process, one can make a popular choice, or instead deviate choosing something unique. In this study, we ask the question:  Are songs that conform to industry norms more successful than those which challenge the status quo? Dependent upon how true this is, then there could be an algorithmic process to writing popular songs, in which case the integrity of musical art is subject to industry danger. For these reasons we will explore if music industry forces have an effect on an artists musical content over time. If there are industry measures being taken to make music fit a particular mold in order to increase popularity, that makes a profoundly dismal statement about appreciation and freedom of artistry. Studying this could give us a warning that art is being treated more as an assembly-line product.
...
1. The functions to get the Billboard Hot 100 [@billboard] ranking data as well as the song lyric data from "Genius.com" were already created by Charlie Liou.  Only minor adjustments were made to these functions. 

2. From Spotify [@Spotify API], first we collected data on artists, to see who qualified to be considered for the study. To qualify, an artist must have: 
...
3. Spotify [@spotify] provided both the top 200 streamed songs by week as well as the song metadata including their metrics of subjective song attributes such as "energy" and "dancability"

4. Grammy winners [@Grammy] of most individual song categories were collected with functionality to specify how far back we wanted to collect data for

5. Certifications of songs (gold, platinum, etc) were collected from the official source of these certifications, the Recording Industry Association of America at "riaa.com"[@RIAA]

6. The music data was collected from a forum based website called "ultimate-guitar.com" [@ultimate guitar] where chord progressions were posted. This data was strongly utilized in the study, but also suffered a few limitations. Because the site was a forum, the way that the data was kept was not always consistent, and the program would only take data which could be validated to match a specific form. This coupled with the fact that this was a guitarist based forum and some genres of music were not equally represented, there was a lot of missing data. 
...
### Popularity
  ...
  This equation is basically communicating that the popularity score will be the sum over all weeks a song was on the charts, of one over that song's position on the Hot 100 that week times the number of weeks the song had been on the charts. This essentially means that for each week the current spot is scaled by the number of weeks the song has been on the charts to that point, and  the scaled value at all weeks is summed. 
  ...
  This equation sums one over the current weekly rank for all weeks a song was on the charts. This was not chosen because we believed that week tenure did matter more than considering each week equally.
...
```{r}
popSep = allArtistReg %>% 
  gather(key = "popMetric", value = "Score", pop1:pop4)

ggplot(popSep, aes(x= popMetric, y = Score)) + 
  geom_boxplot()+ 
  coord_cartesian(ylim = c(0, 20)) + 
  labs(title = "Distributions of each Popularity Metric")
```
...
### Complexity
  ...
  The lyrical complexity of songs excluding stop words (connecting words such as "I", "this", "and", etc) was measured using the following variables:
...
- **Average Syllables:** The mean syllable count of all words in the song. Songs with more words with greater syllable counts may be considered more complex. 
- **Unique Words to Total Word Ratio:** The number of unique words in the song divided by the total number of words in the song. This should give us a measure of lyrical repetition in the song. For this reason, songs with less repetition where less lyrical content is unique and new will be considered less complex than songs with less repetition. 
- **Words Per Second:** The ratio of the number of total words divided by the total number of seconds that was the song's duration. Songs with more words over the total time they are occuring may be considered more complex. 
...
```{r}
ggplot(allArtistReg, aes(x = lyricalComplexity))+ 
  geom_histogram(binwidth = 0.1) +
  labs(title = "Distribution of Lyrical Complexity Scores")
```
  
  From the histogram, it appears that the lyrical complexities are mostly normally distributed. Other than a few outliers, this seems to be true, with the mean being around 11.75. 
  
  The musical complexity of songs we have data on, as there was unfortunately a good amount of missing data, was measured using the following variables:
  
- **Non-Diatonic Chord Count:** The total number of chords in all sections of a song which are outside the key of C major(the key all songs with music data were standardized to). These are chords which we do not expect to see in popular music because the overwhelming popular trend is to use only chords in the key of the song. With this in mind, a frequent pressence of these chords in a song will strongly increase its complexity. 
- **Extended Chord Count:** The total number of chords in all sections of a song which have extra notes in them other than the necessary three notes which make them up. This means that the chord is made of more ingredients than the minimum, giving it more complexity than a non-extended chord. These are not overwhelmingly common, but we may see them sporadically. For these reasons, the pressence of more extended chords will add to the complexity of the song. 
- **Count of Unique Chords:** The total number of unique chords in the song. This is a very important factor in determining song complexity because a song which cycles through a minimal number of chords, will not have as great of a musical vocabulary as a song which introduces many unique chords. For this reason, more unique chords in a song will indicate greater musical complexity. 
- **Count of Sections which End Differently:** The number of unique sections in a song which have an ending chord pattern which does not follow the progression of that section. Songs with more sections that end differently will be more complex because this indicates a lack of true repetition and predictability. 
...
```{r}
ggplot(unstandardizedMusicDf, aes(x = musicalComplexity))+ 
  geom_histogram(binwidth = 5) +
  labs(title = "Distribution of Musical Complexity Scores")
```


There is a very non-uniform distribution of the musical complexity scores, with large peaks sometimes spaced reasonably far from one another. This is because there was so much missing data that we often filled the songs with no music data with the mean of their songs with data. If none of an artists had music data, then their songs would recieve a 1 for musical complexity and this is why there are sporadic peaks, and a peak at 1. 
...
ggplot(archArtistInfluence, aes(x = ReleaseDate, y = nonBandMemberWriters, color = Album, label = Name)) + 
  geom_point() + 
  labs(title = "Number of non-Band Member Writers on Maroon 5's Songs Over Time", y = "Number of Non-Bandmember Writers", x = "Release Date")
```
...
ggplot(lyricalComplexDf, aes(x = ReleaseDate, y = lyricalComplexity, label = Name)) + 
  geom_point(aes(color = Album)) + 
  geom_label_repel(data = subset(lyricalComplexDf, 
                   lyricalComplexity > 2 | lyricalComplexity < -4),
                   box.padding   = 0.35, 
                   point.padding = 0.5, 
                   segment.color = 'grey50', 
                   nudge_y = ifelse(subset(lyricalComplexDf, lyricalComplexity > 2 | lyricalComplexity < -4)$lyricalComplexity > 2, 5, -1)) + 
  theme_classic(base_size = 10) + geom_smooth(method = "lm") + 
  labs(title = "Lyrical Complexity of Maroon 5 Songs by Release Date", y = "Standardized Lyrical Complexity", x = "Release Date")
```
...
From what has been seen so far, it seems that the popularity of Maroon 5 songs has generally risen and complexity has generally decreased as time has passed. The band made all songs internally until about 2012, a point from which they actually started to have a great number of cowriters on projects more and more. They never again had a charting song written without an outside writer. Maybe this could be in part to them switching labels in 2014! When we look at the distribution of the data separated by the different label, the result is as follows. 
...
fullCompleteMetric = join(temp, fullMetric, by = "Name")
fullCompleteMetric = fullCompleteMetric[!is.na(fullCompleteMetric$Label), ]
fullCompleteMetric$Label[fullCompleteMetric$Label == "Interscope"] = "222 Records / Interscope"
```

```{r}
ggplot(fullCompleteMetric, aes(y = pop1, x = Label)) +
  geom_violin(aes(fill = Label), trim = FALSE) + 
  labs(title = "Maroon 5's Song Popularity by Album",x = "Label", y = "Standardized Popularity") + 
  geom_jitter() + 
  coord_flip()

ggplot(fullCompleteMetric, aes(y = nonBandMemberWriters, x = Label)) + 
  geom_violin(aes(fill = Label), trim = FALSE) + 
  labs(title = "Maroon 5's Outside Influence by Album",x = "Label", y = "Number of Non-Bandmember Writers") + 
  geom_jitter()+ 
  coord_flip()

ggplot(fullCompleteMetric, aes(y = totalComplexity, x = Label)) + 
  geom_violin(aes(fill = Label), trim = FALSE) + 
  labs(title = "Maroon 5's Song Complexity by Album", x = "Label", y = "Standardized Total Complexity") + 
  geom_jitter()+ 
  coord_flip()

#grid.arrange(compGraph, popGraph, infGraph, ncol = 2, nrow = 2, top = "Maroon 5's Song Popularity, Complexity, and Outside Influence by Album")
``` 


  Maroon 5's first label was "A&m/Octone", and their second label was the much larger "222 Records/Interscope"(the label after the slash is the parent label). The violin plots above show that greater song complexity was more likely for a song released under Octone than Interscope. Also, greater song popularity was more likely for a song released under Interscope than Octone. Lastly, a song was almost certainly likely to have outside cowriters on it if it was released under Interscope, while it was almost certain that a song would be made completely without outside influence if it was released under Octone. 

```{r}
ggplot(fullCompleteMetric, aes(x= totalComplexity, y =nonBandMemberWriters )) + 
  geom_point(aes(color = Label, size = pop1)) + 
  labs(title= "Number of Non-Band Member Writers, Total Complexity, and Popularity by Record Label", 
       x = "Standardized Total Complexity", 
       y = "Number of Non-Bandmember Writers", 
       size = "Standardized Popularity")
```

  The scatterplot above shows that Octone songs had greater potential for greater complexity than Interscope. Only two charting tracks on Interscope were made with no outside influence, and the songs with the most popularity both had 5 outside cowriters. Both of these songs were right around average complexity for the band. 

When Maroon 5 was on their first record label, A&M/Octone, they only had writing credits on charting songs for one album, "Overexposed", which was the last album they did for A&M/Octone. Some of the least complex songs were created under the new record label, mainly Interscope, with writing credits given to non band members on all of them. Their popularity was reasonably consistent other than "Girls Like you" which was far more popular than all their other songs. While there was decreased complexity in the charting songs which were released on Interscope, the complexity of their charting songs was already in decline from where it started in their first album. There is not a stark decrease in complexity after the record change, but rather a maintenance at the bottom. 
...
ggplot(avgWritersByAlb, aes(x = Album,y= avgPop, fill = avgComplex)) + 
  geom_bar(stat = "identity")+ 
  geom_text(size = 3, aes(label = paste("Avg ",avgnumNonBandWriters, " writers"),vjust=ifelse(avgPop > 0,-0.5,1))) +
  labs(title = "Average Popularity, Complexity, and Non-band Member Writers for each Maroon 5 Album", x = "Album", y = "Average Popularity", fill = "Average Complexity") + 
  scale_x_discrete(limits = albums, labels = c("Songs About Jane", "It Won't be Soon Before Long", "Hands All Over", "Overexposed", "V", "Red Pill Blues")) + 
  theme(axis.text.x = element_text(face="bold", angle=90), axis.text = element_text(size = 6)) # + scale_fill_gradient(low="gray", high="red")
```
...
## Artist Archetypes
  We investigated Maroon 5 and a selection of other artists who qualified for the study as mini case study artists, and each of their popularities, complexities, and outside influence levels over time were compared and contrasted to one another. If artists had similar changes in these metrics, they could potentially be grouped, and it could then be interesting to see if these groups could be clearly separated from one another to form archetypes of some sort. 
  Differences in popularity and complexity therefore can help us discern between types of artists. One difference we see is that over time Maroon 5, Taylor Swift, Justin Bieber, and Britney Spears each gain popularity while Justin Timberlake, Twenty One Pilots, Foo Fighters, and J Cole each lose popularity over time. Another difference between artists is that Foo Fighters, Britney Spears, and J Cole each gain complexity in their charting songs over time while Maroon 5, Twenty One Pilots, and Taylor Swift all lose complexity in their charting songs over time and Justin Timberlake and Justin Bieber both do not see their songs change much in complexity over time. 
  The following had similar patterns in terms of the relationship between popularity and complexity and time. 


* Maroon 5, Taylor Swift: Increase in popularity, decrease in complexity
* J Cole, Foo Fighters: Decrease in popularity, increase in complexity
* All other artists, (Justin Timberlake, Twenty One Pilots, Justin Bieber, Britney Spears): Not the same consistency of popularity and complexity over time as another artist
&nbsp;
  
  We will really be considering the two first bullet point groups (Maroon 5, Taylor Swift & J Cole, Foo Fighters) since they had definitive and similar trajectory paths in popularity and complexity. Now we will see if there is a significant difference between two main groups seen? Let us now look at standardized popularity and complexity over time for both groups on the same plot and test for significance for each of these metrics over time for both groups.
...
```
  ...
  We may ask why groups might behave differently from one another, and if any reasons may have to do with industry forces. As noted in the case study section, Maroon 5 changed labels and then recieved a great number of outside co-writers on their content under the new label, and while Taylor Swift always had outside co-writers, we see increased outside co-writers on her charting songs as time goes on. So the first group has an increasing number of outside co-writers on their songs over time, but what about the second group? Well the Foo Fighters never used any outside co-writers, so their content was created internally, and J Cole does not seem to have a consistent pattern of outside co-writers, but as time goes on he more frequently has songs with fewer outside co-writers. Due to these pieces of information, perhaps aside from less quantifiable factors such as genre or period of popularity, the outside influence on charting songs increases over time for Maroon 5 and Taylor Swift while songs are made with less outside input over time in J Cole and the Foo Fighters' music.
...
ggplot(allArtistRegSub, aes(x = totalComplexity, y = pop1)) +
  geom_point(aes(color = Artist)) +
  stat_smooth(method = "lm")
```
...
```{r}
chart.Correlation(allArtistRegSub[c("musicalComplexity", "lyricalComplexity")])
ggplot(allArtistRegSub, aes(x = musicalComplexity, y = lyricalComplexity)) +
  geom_point(aes(color = Artist)) +
  stat_smooth(method = "lm")
```
  
  The correlation between musical complexity and lyrical complexity has strong evidence of being significant with a correlation coefficient of -0.29. There are a lot of filled values however as there was a lot of missing data, so this would be more reliable if the dataset was less artificial. 
...
- *Do particular labels try to change artists more than others?* If we developed a measure of change by taking the difference of the popularity, influence, and complexity metrics then we could map the change of artists by label. 

- *Do music industry forces try to push artists towards certain genres?* Could look at artists who have experienced genre change over time and see if they have aquired more writers on their content or if they have changed labels, etc.

- *Is wordier(music with more lyrics) music more popular or less popular than more instrumental music?* The spotify metrics of 'speechiness' and 'instrumentalness' could be coupled with the lyrical and musical complexity metrics to see how speechier songs with varying levels of complexity compare to more instrumental songs. Binning could be done by a combination of these two scores. 

- *Do can we predict for newer artists what archetype they are most likely to be a part of?* Machine learning could be used to train a model based on the behavior of more tenured artists to predict how an emerging arist would develop based on the data that exists on them. 

- *What are the properties of certain archetypes in the short and long term in terms of popularity and autonomy?* Can cluster artists, and then extropolate separately on the metrics based on the data of all artist's trends in the cluster. 
