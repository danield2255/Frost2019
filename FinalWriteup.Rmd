...
## Introduction
  John Seabrook's "The Song Machine: Inside the Hit Factory" portrays the modern music industry as a machine which has been programmed to make money and breed songwriters and artists to make uniform choices in order to maintain some success algorithm. The book claims "[a] common problem hit factories suffer is that the balance of power between the artists and the writers and producers tends to change over time," suggesting that the relationships between artists and music business entities can alter over time, potentially being driven by different motives. So one might ask if in all of this volatility and evolution in the relationships that the music is affected? The industry norms that the book suggests can be considered and studied in the context of data by finding ways to measure the popularity, complexity, and outside influence on a song. Ultimately, this data can be used to try to answer the question “What is the effect over time of music industry influence on musical artists, in terms of musical content?”
  
  Subjective measures will be difficult to quantify reliably but there is evidence in pre-existing data which can help make some strong arguments. Essentially, just because a right answer cannot be verified, this does not mean that attempts can't help make valid conclusions. Scaling results to the entire music industry will be difficult and impractical, but a look into some case study artists will give some insights to how choices to make complex music can affect popularity, and potentially how outside influence can affect both complexity and popularity. Perhaps with these relationships explored, then the long term effect of music industry influence on music content can be better defined. 
...
## Data Collection
  ...
  1. The functions to get the Billboard Hot 100 ranking data as well as the song lyric data from "Genius.com" were already created by Charlie Lui. (this is his last name correct?) Only minor adjustments were made to these functions. 
...
### Popularity
  Quantifying popularity is subjective, but a metric will be used that accounts for the each week's position on the chart scaled by the number of weeks it has been on the charts. This rewards longevity on the charts, and heavily rewards higher positions. This will be 
  $$pop1 = \sum(\frac{weeksOnBillboard}{currentBillboardRank})$$
For each week the current spot is scaled by the number of weeks the song has been on the charts to that point, and have the scaled value at all weeks summed.
```{r}
#plot of pop1 distribution
ggplot(allArtistReg, aes(x = pop1))+ geom_histogram(binwidth = 5) +labs(title = "Distribution of Pop1 Scores")
```

  Other metrics were considered and they while they were not chosen to be applied to the rest of the study, they are documented below. 
