...
#ggplot(archArtistInfluence, aes(x = ReleaseDate, y = nonBandMemberWriters)) + geom_point()
```
...
# str_replace("Hi Daniel", list(a= "x", i =  "z")) ## doesn't work
...
ggplot(lyricalComplexDf, aes(x = ReleaseDate, y = lyricalComplexity)) + geom_point() + geom_smooth(method = "lm")
```
...
ggplot(musicComplexDf, aes(x = ReleaseDate, y = musicalComplexity)) + geom_point() + geom_smooth(method = "lm")
```
...
artistMetricDf %>%
  ...
  ggplot(aes(x = Name, y = Score, fill = Metric)) + geom_col(position = "dodge")
```
